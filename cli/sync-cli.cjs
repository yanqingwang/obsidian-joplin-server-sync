  ...
  "src/vault/VaultWatcher.ts"() {
    ...
    VaultWatcher = class {
      ...
      async record(kind, path4, oldPath, isFolder, file) {
        ...
        let fileId;
        if (kind === "delete") {
          fileId = this.plugin.mapping.getByPath(path4)?.joplinId ?? "file:" + path4;
        } else {
...
var init_DeltaPuller = __esm({
  "src/core/DeltaPuller.ts"() {
    ...
    init_pathUtil();
    init_FileIdentity();
    DeltaPuller = class _DeltaPuller {
      // item_id → full path
      constructor(plugin, watcher) {
        this.plugin = plugin;
        this.watcher = watcher;
      ...
      async pullAll() {
        ...
        if (totalMapped > 20 && deletes.length > totalMapped / 2) {
          ...
          new Notice("Large delete batch (" + deletes.length + ') from server. Applying with verification \u2014 run "Force pull" if local state diverged.', 1e4);
        }
    ...
    SyncEngine = class {
      ...
      async forcePullInner() {
        try {
          ...
          const readStats = remoteStats.filter(
            (s) => /^[0-9a-f]{32}\.md$/.test(s.name) && !s.name.startsWith(".resource/")
          );
          ...
          for (const batch of chunk2(readStats, 5)) {
            ...
            const results = await Promise.all(batch.map(async (stat) => {
              try {
                ...
                failed++;
                if (failed <= 3)
                  console.error("[joplin-sync] force-pull read:", stat.name, e);
                return null;
              }
            }));
          ...
          const foldersByDepth = /* @__PURE__ */ new Map();
          for (const f of folders) {
          ...
          for (const batch of chunk2(notes, 5)) {
            await Promise.all(batch.map(async (item) => {
              ...
              try {
                ...
                failed++;
                if (failed <= 3)
                  console.error("[joplin-sync] force-pull note:", item.title, e instanceof Error ? e.message : String(e));
              }
            }));
...
init_obsidian_real();
var JoplinServerApi = class _JoplinServerApi {
  ...
  async rawRequest(method, path4, opts = {}) {
    ...
    for (let attempt = 0; ; attempt++) {
      ...
      try {
        const res = await this.requestWithTimeout(method, path4, headers, opts.body, timeoutMs);
        if (res.status === 401) {
          this.sessionId = null;
        ...
        if (attempt < maxRetries) {
          console.warn("[joplin-sync] request failed (attempt " + (attempt + 1) + "/" + (maxRetries + 1) + "), retrying: " + method + " " + path4 + " \u2014 " + (e instanceof Error ? e.message : String(e)));
          await this.sleep(Math.pow(2, attempt) * 1e3);
          continue;
        }
        throw e;
  ...
  requestWithTimeout(method, path4, headers, body, timeoutMs) {
    const url = this.trimSlash(this.getConfig().baseUrl) + path4;
    ...
    const timeout = new Promise((_, reject) => {
      timer = window.setTimeout(
        () => reject(new Error("Request timed out after " + timeoutMs + "ms: " + method + " " + path4)),
        timeoutMs
      );
    });
